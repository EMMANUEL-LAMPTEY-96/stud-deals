...
export const StampSchema = z.object({
  vendor_id: uuid,
  /**
   * TOTP-style time-window nonce for QR replay protection (introduced in task #110).
   * Required — QR codes generated by VendorQRPanel always embed the 5-minute bucket:
   *   Math.floor(Date.now() / (5 * 60 * 1000))
   * Omitting the nonce is rejected so that raw API calls cannot bypass replay protection.
   */
  nonce: z.string().min(1, { message: 'nonce is required for replay protection.' }),
});
