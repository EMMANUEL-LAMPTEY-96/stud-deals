...
export async function POST(request: NextRequest) {
  ...
  // ── 2. Global rate limit: max 50 stamp requests per student per hour ─────────
  // The per-vendor 8-hour cooldown (checked in step 5) is the primary guard.
  // This secondary global limit prevents scripted multi-vendor abuse.
  const admin = createAdminClient();
  const hourAgo = new Date(Date.now() - 60 * 60 * 1000).toISOString();

  // Look up student profile first (needed for rate limit + verification check)
  const { data: studentProfileForLimit } = await admin
    .from('student_profiles')
    .select('id, verification_status')
    .eq('user_id', user.id)
    .maybeSingle();

  if (studentProfileForLimit) {
    const { count: stampsThisHour } = await admin
      .from('redemptions')
      .select('id', { count: 'exact', head: true })
      .eq('student_id', studentProfileForLimit.id)
      .in('status', ['stamp', 'reward_earned', 'tier_reward'])
      .gte('confirmed_at', hourAgo);

    if ((stampsThisHour ?? 0) >= 50) {
      return NextResponse.json(
        { error: 'Too many stamp requests this hour. Please try again later.' },
        {
          status: 429,
          headers: {
  ...
  const { vendor_id, nonce } = parsed.data;

  // ── 4. Nonce / replay protection ───────────────────────────────────────────
  // QR codes generated by VendorQRPanel embed the 5-minute bucket:
  //   Math.floor(Date.now() / (5 * 60 * 1000))
  // The nonce is required — requests without it are rejected to prevent
  // replay attacks where an attacker re-submits a captured vendor_id repeatedly.
  {
    const nonceWindow = parseInt(nonce, 10);
    const currentWindow = Math.floor(Date.now() / (5 * 60 * 1000));
    const windowDrift = Math.abs(currentWindow - nonceWindow);

    if (isNaN(nonceWindow) || windowDrift > 1) {
      return NextResponse.json(
        {
          error: 'This QR code has expired. Please ask the vendor to show a fresh QR code.',
  ...
  // ── 4. Look up the student profile (reuse from rate-limit check if available) ─
  // The rate-limit step above already fetched this; re-fetch only if it was skipped.
  const { data: studentProfile } = studentProfileForLimit
    ? { data: studentProfileForLimit }
    : await admin
        .from('student_profiles')
        .select('id, verification_status')
        .eq('user_id', user.id)
  ...
  // ── VULN-06 fix: Verified students only ───────────────────────────────────
  if (studentProfile.verification_status !== 'verified') {
    return NextResponse.json({
      error: 'Student verification required to earn stamps. Please verify your student status first.',
  ...
  insertRows.push({
    ...
    status:          mainStatus,
    redemption_code: secureStampCode('STAMP'),
    claimed_at:      now,
    confirmed_at:    now,
  ...
  if (inDoubleWindow) {
    insertRows.push({
      ...
      offer_id:        targetOffer.id,
      status:          'stamp',
      redemption_code: secureStampCode('STAMP-DOUBLE'),
      claimed_at:      now,
      confirmed_at:    now,
  ...
  for (let i = 0; i < bonusStamps; i++) {
    insertRows.push({
      ...
      offer_id:        targetOffer.id,
      status:          'stamp',
      redemption_code: secureStampCode('STAMP-BONUS'),
      claimed_at:      now,
      confirmed_at:    now,
...
//   - Students may only query their own student_profile_id.
//   - Vendors may query any student_id for their own vendor_id (needed by LoyaltyScanner).
//   - Admins may query any combination.
