...
export async function POST(request: NextRequest) {
  ...
  // Look up student profile first (needed for the count query)
  ...
  // If the QR code includes a time-window nonce (?t=XXXXX), verify it is within
  // ±1 window of the current time (i.e. within ~10 minutes).
  // QR codes generated by VendorQRPanel embed the 5-minute bucket:
  //   Math.floor(Date.now() / (5 * 60 * 1000))
  // Nonces are optional — scans without a nonce are accepted to allow direct
  // API calls and legacy QR codes. Future versions may enforce nonce presence.
  if (nonce !== undefined) {
